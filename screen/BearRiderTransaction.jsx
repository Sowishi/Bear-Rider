...
const BearRiderTransaction = ({ navigation }) => {
  const [transactions, setTransactions] = useState([]);
  const [refreshing, setRefreshing] = useState(false);
  const { currentUser } = useSmokeContext();
  const [filter, setFilter] = useState("All");
  const [sortOrder, setSortOrder] = useState("desc");
  const { getRiderTransaction } = useCrudTransaction();
  ...
  useEffect(() => {
    getRiderTransaction(currentUser.id, setTransactions);
  }, []);
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <TouchableOpacity
          onPress={() => navigation.navigate("RiderEarning", { transactions })}
          style={{ justifyContent: "center", alignItems: "center" }}
        >
          <FontAwesome6 name="peso-sign" size={24} color="black" />
          <Text>Earnings</Text>
        </TouchableOpacity>
      </View>
...
export default BearRiderTransaction;
